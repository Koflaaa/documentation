...
API connectors enable Odoo Sales to send and receive data from Gelato for order processing, while
webhooks provide real-time updates on order status and shipment tracking.
...
To create a webhook, go to :menuselection:` Developer --> Webhooks` in the left menu bar. In the new
page, click :guilabel:`Add Webhook` to open a :guilabel:`Create Webhook` form.

The webhook form requires several specific configurations:

- :guilabel:`URL`: Copy and paste your Odoo database URL with the additional suffix
  "/gelato/webhook".
...
To create a product in Odoo that matches the Gelato one, navigate to :guilabel:`Sales app -->
Products --> Products`, select New to create a new product form. Type in the product
:guilabel:`Name`, then navigate to the :guilabel:`Sales` tab. Find the :guilabel:`Gelato` field,
