...
The Odoo **Data Cleaning** app keeps the database neat and organized with its provided features:
...
Through customizable rules, text data in the database remains up-to-date, deduplicated, consistently
formatted, and tailored to company specifications.
...
.. tip::
   The recycle rules run once a day, by default, as part of a scheduled action cron (*Data Recycle:
...
- :guilabel:`Transient Model`: the model handles temporary data that does not need to be persisted
  in the database.
- :guilabel:`Can Be Merged`: enables the *Merge* action for the model.

To view the which models are enabled by default, use the :ref:`search bar <search/filters>` to
filter models that :guilabel:`Can Be Merged`.
